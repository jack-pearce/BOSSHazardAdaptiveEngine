...
static boss::expressions::ExpressionSpanArgument toBOSSExpression(ExpressionSpanArgument&& span) {
  return std::visit(
      []<typename T>(boss::Span<T>&& typedSpan) -> boss::expressions::ExpressionSpanArgument {
        if constexpr(std::is_same_v<std::decay_t<T>, Pred>) {
          throw std::runtime_error("there must be no Span<Pred>");
        } else {
  ...
  for(; it != std::move_iterator(dynamics.end()); ++it) {
    if(!std::visit(
           [&spanInputs]<typename InputType>(InputType&& argument) {
             ...
             if constexpr(boss::utilities::isVariantMember<std::vector<Type>, boss::SpanInputs>::value) {
               if(!spanInputs.empty() &&
...
public:
  OperatorMap() {
    (*this)["Plus"_] =
      ...
      return visitAccumulate(std::move(args), static_cast<FirstArgument>(0), [](auto&& state, auto&& arg) {
...
//    };
    ...
    (*this)["Multiply"_] =
        []<NumericType FirstArgument>(
            ComplexExpressionWithStaticArguments<FirstArgument>&& input) -> Expression {
      ExpressionArguments args = input.getArguments();
      return visitAccumulate(std::move(args), static_cast<FirstArgument>(1), [](auto&& state, auto&& arg) {
        if constexpr(NumericType<std::decay_t<decltype(arg)>>) {
          state *= arg;
        }
        return state;
    ...
    (*this)["Select"_] = [](ComplexExpression&& inputExpr) -> Expression {
      ...
      std::cout << "Expression: " << inputExpr << std::endl;
      ...
      std::cout << "Predicate: " << predExpr << std::endl;

//      auto predFunc = boss::get<Pred>(std::move(predExpr)); // TODO
      auto columns = std::move(relation).getDynamicArguments();

//      std::cout << "Pred function: " << predFunc << std::endl;
      ...
      //https://github.com/symbol-store/BOSSArrayFireEngine/blob/support_tpch_q6/Source/BOSSArrayFireEngine.cpp
      // Use where array and span index from the link above in the code below

      std::transform(
          std::make_move_iterator(columns.begin()), std::make_move_iterator(columns.end()),
          columns.begin(), [](auto&& columnExpr) {
            // for each column, filter (in-place) the span with the predicate
            auto column = get<ComplexExpression>(std::forward<decltype(columnExpr)>(columnExpr));
            auto [head, unused_, dynamics, spans] = std::move(column).decompose();
            auto list = get<ComplexExpression>(std::move(dynamics.at(0)));
            auto [listHead, listUnused_, listDynamics, listSpans] = std::move(list).decompose();
            listSpans.at(0) = std::visit(
                []<typename T>(boss::Span<T>&& typedSpan) -> ExpressionSpanArgument {
                  if constexpr(NumericType<std::decay_t<T>>) {
                    auto predFunc = []<typename T2>(T2 value) -> bool { return value > 3; };

                    auto result = std::vector<std::decay_t<T>>();
                    for (auto inputValue : typedSpan) {
                      if (predFunc(inputValue)) {
                        result.push_back(inputValue);
                      }
                    }
                    return boss::Span<T>(std::move(std::vector(result)));
                  } else {
...
private:
  ...
  static Pred createLambdaExpression(ComplexExpressionWithStaticArguments<T>&& e, F&& f) {
    ...
    if constexpr(MAX_ARGS == 1) {
      auto pred = [pred1 = createLambdaArgument(get<0>(e.getStaticArguments())),
        ...
        return (ExpressionSpanArgument)f(*arg1);
      };
      ...
      auto pred = std::visit(
          [&e, &f](auto&& arg) -> Pred::Function {
            return [pred1 = createLambdaArgument(get<0>(e.getStaticArguments())),
              ...
              return (ExpressionSpanArgument)f(*arg1, *arg2);
            };
          ...
          [&f](auto&& acc, auto const& e) -> Pred::Function {
            return std::visit(
                [&e, &f, &acc](auto&& arg) -> Pred::Function {
                  auto pred2 = createLambdaArgument(arg);
                  return
                      [acc, pred2, f](ExpressionArguments& columns) mutable -> std::optional<ExpressionSpanArgument> {
                        auto arg1 = acc(columns);
                        auto arg2 = pred2(columns);
                        if(!arg1 || !arg2) {
                          return {};
  ...
  template <typename ArgType> static auto createLambdaArgument(ArgType const& arg) {
    if constexpr(NumericType<ArgType>) {
      return [arg](ExpressionArguments& /*unused*/) -> std::optional<ArgType> { return arg; };
    } else {
      throw std::runtime_error("unsupported argument type in predicate");
      return [](ExpressionArguments& /*unused*/) -> std::optional<bool> { return {}; };
    }
  }
  ...
  static Pred::Function createLambdaArgument(Symbol const& arg) {
    return [arg, index = 0U - 1U](ExpressionArguments& columns) mutable -> std::optional<ExpressionSpanArgument> {
      ++index;
      ...
      for(auto& columnExpr : columns) {
        ...
        if(get<Symbol>(column.getArguments().at(0)) == arg) {
          if(index >= get<ComplexExpression>(column.getArguments()[1]).getSpanArguments().size()) {
            return {};
          }
          ...
          auto [listHead, listUnused_, listDynamics, listSpans] = std::move(list).decompose();
          auto array = std::visit(
              [&arg]<typename T>(boss::Span<T>& typedSpan) -> std::optional<ExpressionSpanArgument> {
                if constexpr(boss::utilities::isVariantMember<std::vector<T>,
                                                              boss::SpanInputs>::value)  {
                  return static_cast<ExpressionSpanArgument>(std::move(typedSpan));
                } else {
...
//  return std::visit(boss::utilities::overload(std::move(function), [](auto&& atom) -> Expression { return atom; }),
//                    std::move(expression));
//}
